...
def std_list_read_file():
    if os.path.isfile(file_name):
        with open('json_student_data.txt', 'r', encoding='utf-8') as f:
...
def show_std_list(students):
    print(type(students))       # list타입
    print(students)
    for std in students:
        print(type(std))        # dict타입
...
def std_list_write_file(students):
    json_student = json.dumps(students, ensure_ascii=False)  # json 형태로 변환
    with open('json_student_data.txt', 'w', encoding='utf-8') as f:
        json.dump(json_student, f, ensure_ascii=False)  # json 파일 작성 한굴은 unicode형태로 저장
...
def update_std_info(students):
    ...
    for std in students:
        if int(res) == std['no']:
            print("해당학생 존재")
            std_info = get_std_info("수정할 국어 영어 수학을 입력하세요.>>")
